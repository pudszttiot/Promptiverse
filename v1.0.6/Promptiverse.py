...
class EditPromptDialog(QDialog):
    def __init__(self, prompt_text, parent=None):
        ...
        layout = QVBoxLayout(self)
        prompt_label = QLabel("Edit Prompt:")
...
class PromptManager(QWidget):
    ...
    def edit_prompt(self):
        ...
        try:
            self.cursor.execute("SELECT prompt FROM prompts WHERE title=?", (title,))
            prompt_text = self.cursor.fetchone()[0]
            dialog = EditPromptDialog(prompt_text, self)
            edited_prompt = dialog.get_edited_prompt()
            if edited_prompt:
                self.cursor.execute(
                    "UPDATE prompts SET prompt=? WHERE title=?", (edited_prompt, title)
                )
                self.connection.commit()
